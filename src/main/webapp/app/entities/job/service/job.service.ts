...
type RestOf<T extends IJob | NewJob> = Omit<T, 'dateFrom' | 'dateTo'> & {
...
export class JobService {
  ...
  protected convertDateFromClient<T extends IJob | NewJob | PartialUpdateJob>(job: T): RestOf<T> {
    return {
      ...job,
      dateFrom: job.dateFrom?.toJSON() ?? null,
      dateTo: job.dateTo?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restJob: RestJob): IJob {
    return {
      ...restJob,
      dateFrom: restJob.dateFrom ? dayjs(restJob.dateFrom) : undefined,
      dateTo: restJob.dateTo ? dayjs(restJob.dateTo) : undefined,
    };
  }
