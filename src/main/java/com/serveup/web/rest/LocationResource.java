...
import com.serveup.repository.LocationRepository;
...
/**
 * REST controller for managing {@link com.serveup.domain.Location}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class LocationResource {
    ...
    public List<Location> getAllLocations(@RequestParam(required = false) String filter) {
        if ("job-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where job is null");
            return StreamSupport
                .stream(locationRepository.findAll().spliterator(), false)
                .filter(location -> location.getJob() == null)
