...
import com.serveup.repository.JobRepository;
...
/**
 * REST controller for managing {@link com.serveup.domain.Job}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class JobResource {
     ...
     */
    @PutMapping("/jobs/{id}")
    public ResponseEntity<Job> updateJob(@PathVariable(value = "id", required = false) final UUID id, @RequestBody Job job)
    ...
    public ResponseEntity<Job> partialUpdateJob(@PathVariable(value = "id", required = false) final UUID id, @RequestBody Job job)
        throws URISyntaxException {
        ...
        Optional<Job> result = jobRepository
            ...
            .map(existingJob -> {
                ...
                if (job.getDificulty() != null) {
                    existingJob.setDificulty(job.getDificulty());
                }
                if (job.getDateFrom() != null) {
                    existingJob.setDateFrom(job.getDateFrom());
                }
                if (job.getDateTo() != null) {
                    existingJob.setDateTo(job.getDateTo());
                }
                ...
                if (job.getTypeOfJob() != null) {
                    existingJob.setTypeOfJob(job.getTypeOfJob());
                }
